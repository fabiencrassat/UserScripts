...
const checkResponseAndUrl = function(response, url) {
    return response.ok && response.status === 200 && url && url.startsWith(apiUrlToFollow);
};

const response = function(response) {
    const responseClone = response.clone(); // clone to consume json body stream response    
    const url = response.url;
    if (checkResponseAndUrl(responseClone, url)) {
